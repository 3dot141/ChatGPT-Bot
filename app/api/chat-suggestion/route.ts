...
function createSuggestionPromptTemplate(input: string): string {
  return `
  你是一个内容转化器，下面我会给你一段内容，你要按照规则转换这段内容为三个问题。规则：将内容转化为三个问题：1、内容的排查方案是什么，2、内容的解决方法是什么，3、内容的排查思路是什么。请思考内容的替换方式，但是排查方案、解决方法、排查思路不会丢失。你要考虑转化的合理性，即当前的内容在语义上是否需要排查方案、解决方法、排查思路。不是，则返回空数组。是只需要返回三个问题的数组即可。返回内容需要非常简洁，只需要返回 json 格式的回答。不需要额外的信息。
example
input: 目录权限设置不生效 
output: 
{
